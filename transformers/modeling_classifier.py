...
class MultiCNNFCLayerClassifier(nn.Module):
    def __init__(self, config, **kwargs):
        super(MultiCNNFCLayerClassifier, self).__init__()
        ...
        self.fc1 = nn.Linear(self.fc_size, 300)
        self.fc2 = nn.Linear(self.fc_size, 300)
    ...
    def forward(self, bert_last_layer, cnn_last_layer, labels):
        ...
        if self.cnn_train:
            ...
            # Concatenate BERT at each of the three fc layers of CNN
            x = self.fc1(concat_emb)
            concat_emb = torch.cat([bert_last_layer, x], 1)

            x = self.fc2(concat_emb)
            concat_emb = torch.cat([bert_last_layer, x], 1)
        else:
            concat_emb = bert_last_layer
